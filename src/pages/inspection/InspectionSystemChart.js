...
// // // //   *  - "기준 ± 공차"        → 예: "9.0 ±0.2"
// // // //   *  - "저~고"              → 예: "8.8~9.2"
// // // //   *  - "Ø기준,(0,+0.2) 이내" → 예: "Φ9.0 (0, +0.2mm 이내)"  → [9.0, 9.2] (일방 상한 공차)
// // // //   *  - "Ø기준, +0.2mm 이내" → 예: "ø13.5, +0.2mm 이내"     → [13.5, 13.7] (일방 상한 공차)
// // // //   */
...
// // // //   if (m) {
// // // //     const a = toNum(m[1]); const b = toNum(m[2]);
...
// // // //       const l = Math.min(low, high), h = Math.max(low, high);
// // // //       return { low: l, high: h, label: `${fmtNum(l, 3)} ~ ${fmtNum(h, 3)}` };
// // // //     }
// // // //   }

// // // //   // 4) plus-only tolerance text: "ø13.5, +0.2mm 이내" → [13.5, 13.7]
...
// // // //   // 5) (구) 대칭 가정 패턴은 더 이상 사용하지 않지만, 호환을 위해 남겨둠
// // // //   m = t.match(/(?:ø|Ø|Φ)?\s*([-+]?\d+(?:\.\d+)?)\D+\+?([-+]?\d+(?:\.\d+)?)/i);
// // // //   if (m) {
// // // //     const center = toNum(m[1]); const tol = toNum(m[2]);
// // // //     if (!Number.isNaN(center) && !Number.isNaN(tol)) {
// // // //       // 상한 이내로 쓰일 가능성이 높아 [center, center+tol]로 해석
// // // //       const low = center;
// // // //       const high = center + tol;
// // // //       return { low, high, label: `${fmtNum(low, 3)} ~ ${fmtNum(high, 3)}` };
// // // //     }
...
// // // //   /** ✅ Y축 tick을 고정 간격(기본 0.1)으로 생성 (클래스 메서드 버전) */
...
// // // //     const cards = this.buildConcatXnSeriesPerSpec();

// // // //     return (
// // // //       <Paper className={`${s.section} ${s.fullRow}`} style={{ marginTop: 16 }}>
...
// // // //             <CircularProgress size={44} sx={{ color: "#ff8f00" }} />
// // // //           </Box>
// // // //         ) : !cards.length ? (
...
// // // //               // ✅ 날짜 5개만 한 화면에: 내부 차트 픽셀 폭 계산
// // // //               const shifts = Math.max(1, (this.state.dailyShifts?.length || 0));
// // // //               const xns = Math.max(1, (this.state.dailyCols?.length || 0));
// // // //               const pointsPerDay = shifts * xns; // 주/야 × Xn 개수
// // // //               const minVisibleWidth = DAY_WINDOW * pointsPerDay * TICK_WIDTH;
// // // //               const fullWidth = Math.max(minVisibleWidth, data.length * TICK_WIDTH);

// // // //               // ✅ 날짜 변경 지점만 X축 눈금으로 표시
// // // //               const dayTicks = [];
// // // //               let lastDay = null;
// // // //               for (const p of data) {
// // // //                 if (p.day !== lastDay) {
// // // //                   dayTicks.push(p.idx); // dataKey="idx"
// // // //                   lastDay = p.day;
// // // //                 }
// // // //               }

// // // //               // ✅ 카드 우측에 품번/품명/기간 칩 표시
// // // //               const partText = this.state.filters.partNo || "전체 품번";
// // // //               const itemText = this.state.filters.item || "";
// // // //               const rangeText = `${this.state.filters.start_date || "-"} ~ ${this.state.filters.end_date || "-"}`;

// // // //               // ✅ 스펙 범위 파싱
// // // //               const specRange = parseSpecRange(specText);
// // // //               const yDomain = this.getPaddedYDomain(
// // // //                 data,
// // // //                 ["y"],
// // // //                 0.1,
// // // //                 specRange ? [specRange.low, specRange.high] : []
// // // //               );
...
// // // //                       <Chip size="small" label={partText} />
// // // //                       {itemText && <Chip size="small" variant="outlined" label={itemText} />}
// // // //                       <Typography sx={{ color: "#607d8b", fontWeight: 800, fontSize: 12, ml: 0.5 }}>기간</Typography>
// // // //                       <Chip size="small" label={rangeText} />
// // // //                     </Box>
// // // //                   </Box>
...
// // // //                         <YAxis
// // // //                           domain={yDomain}
// // // //                           ticks={this.makeTicksByStep(yDomain, 0.1)}   // ✅ 0.1 간격 고정
// // // //                           allowDataOverflow
// // // //                           tickFormatter={(v) => fmtNum(v, 1)}          // 보기 좋게 소수 1자리
// // // //                         />
...
// // // //                           }}
// // // //                         />
// // // //                         <Line type="monotone" dataKey="y" name="값" dot={{ r: 3 }} activeDot={{ r: 6 }} strokeWidth={2} connectNulls>
// // // //                           <LabelList
// // // //                             dataKey="y"
// // // //                             content={(props) => {
...
// // // //                       X축은 각 날짜마다 <b>주간 X1→Xn</b>을 먼저, 이어서 <b>야간 X1→Xn</b> 순서로 붙입니다. 한 화면에는 보고일 5개씩 보이며, 날짜가 바뀌는 지점에서만 눈금이 표시됩니다.
// // // //                     </Typography>
// // // //                     {specRange && (
...
// // // //   render() {
// // // //     const { error, dailyList, selectedDay, loadingDaily, dayPanelHeight } = this.state;
...
// // //       try { this._chartRO.unobserve(this._chartViewportEl); } catch {}
// // //     }
// // //     this._chartViewportEl = el;

// // //     if (el && typeof ResizeObserver !== "undefined") {
// // //       if (!this._chartRO) {
// // //         this._chartRO = new ResizeObserver(() => this.updateChartViewportWidth());
// // //       }
...
// // //     window.addEventListener("resize", this.updateDayPanelHeight);
// // //     window.addEventListener("resize", this.updateLeftPanelHeight);
// // //     // 창 크기 변화에도 차트 뷰포트 갱신(일부 브라우저는 사이드바 토글 시에도 발생)
// // //     window.addEventListener("resize", this.updateChartViewportWidth);
// // //   }

// // //   componentWillUnmount() {
// // //     window.removeEventListener("resize", this.updateDayPanelHeight);
// // //     window.removeEventListener("resize", this.updateLeftPanelHeight);
// // //     window.removeEventListener("resize", this.updateChartViewportWidth);
// // //     if (this._chartRO) {
// // //       try { this._chartRO.disconnect(); } catch {}
// // //       this._chartRO = null;
// // //     }
// // //   }
...
// // //       this.updateDayPanelHeight();
// // //       this.updateLeftPanelHeight();
// // //       this.updateChartViewportWidth(); // 데이터 로드 후 한 번 더 보정
// // //     });
// // //   };
...
// // //               // 최소 보장(5일) vs 실제 데이터 필요폭 vs 가용 뷰포트 폭 중 최댓값 사용
// // //               const minVisibleWidth = Math.max(
// // //                 viewportW,
// // //                 DAY_WINDOW * pointsPerDay * TICK_WIDTH
// // //               );
...
// // //                   {/* 가로 스크롤 컨테이너 (첫 카드에만 ref 걸어 가용폭 관찰) */}
// // //                   <Box
// // //                     sx={{ width: "100%", overflowX: "auto", overflowY: "hidden" }}
// // //                     ref={i === 0 ? this.setChartViewportRef : undefined}
// // //                   >
// // //                     {/* 내부 고정폭 박스 (가로 스크롤 대상) */}
// // //                     <Box sx={{ width: `${fullWidth}px`, height: 300 }}>
// // //                       <LineChart width={fullWidth} height={300} data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
...
// // //                         <YAxis
// // //                           domain={yDomain}
// // //                           ticks={this.makeTicksByStep(yDomain, 0.1)}
// // //                           allowDataOverflow
...
// //       }
// //       this._chartRO.observe(el);
// //       this.updateChartViewportWidth(); // 최초는 즉시 1회
// //     }
// //   };
...
// //     window.addEventListener("resize", this.updateDayPanelHeight);
// //     window.addEventListener("resize", this.updateLeftPanelHeight);
// //     // 창 크기/사이드바 토글 등에 따른 가용폭 변화를 디바운스 반영
...
// //                           strokeWidth={2}
// //                           connectNulls
// //                           isAnimationActive={false}   // ← 깜빡임 방지: 애니메이션 off
// //                         >
// //                           <LabelList
// //                             dataKey="y"
// //                             isAnimationActive={false}  // ← 라벨 애니메이션도 off
// //                             content={(props) => {
// //                               const { x, y, value } = props;
// //                               if (value == null || Number.isNaN(Number(value))) return null;
// //                               return (
...
//     // 2) 데이터 없는 카드 제외 + 특정 키워드 포함 라벨 제외
    ...
    return (
      <Paper className={`${s.section} ${s.fullRow}`} style={{ marginTop: 16 }}>
        <Box className={s.sectionHeader} sx={{ alignItems: "center" }}>
          ...
          {/* ※ 섹션 헤더에서는 칩을 표시하지 않습니다. (각 차트 카드에 표시) */}
        </Box>
